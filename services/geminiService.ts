...
export const explainText = async (text: string, style: ExplainStyle): Promise<string> => {
    ...
    try {
        const prompt = `Analyze the language of the following text and provide your explanation in that SAME language.

Explain the text in a ${style} manner.
...
export const queryWithContext = async (query: string, context: string): Promise<string> => {
    ...
    try {
        const prompt = `Analyze the language of the user's question and respond in that SAME language.

Based ONLY on the following context from a user's notes, answer the user's question.
